...
class Video_Highlight_Slider_Widget extends Widget_Base {
	...
	protected function register_controls() {
		...
		$repeater->add_control( 'button_text', array(
			'label'       => esc_html__( 'Button Text / Label', 'evhs' ),
			'type'        => Controls_Manager::TEXT,
			'placeholder' => esc_html__( 'Watch Now', 'evhs' ),
		) );
		...
		$repeater->add_control( 'link', array(
			'label'       => esc_html__( 'Optional Link', 'evhs' ),
			'type'        => Controls_Manager::URL,
	...
	private function get_default_videos() {
		$base = trailingslashit( plugin_dir_url( dirname( __DIR__ ) . '/elementor-video-highlight-slider.php' ) . 'videos' );

		return array(
			array( 'video_source' => 'url', 'video_url' => array( 'url' => $base . 'video1.mp4' ), 'title' => 'Momentum Sprint', 'subtitle' => 'Training Highlight' ),
			array( 'video_source' => 'url', 'video_url' => array( 'url' => $base . 'video2.mp4' ), 'title' => 'Arena Energy', 'subtitle' => 'Matchday Feature' ),
			array( 'video_source' => 'url', 'video_url' => array( 'url' => $base . 'video3.mp4' ), 'title' => 'Peak Performance', 'subtitle' => 'Team Showcase' ),
		);
	}
	...
	protected function render() {
		...
		<div class="evhs-wrapper evhs-loading">
			<div class="evhs-video-slider swiper" data-autoplay="<?php echo esc_attr( $settings['autoplay'] ); ?>" data-autoplay-delay="<?php echo esc_attr( $settings['autoplay_delay'] ?? 3500 ); ?>" data-speed="<?php echo esc_attr( $settings['speed'] ?? 1000 ); ?>" data-space-desktop="<?php echo esc_attr( $space ); ?>" data-space-tablet="<?php echo esc_attr( $space_tablet ); ?>" data-space-mobile="<?php echo esc_attr( $space_mobile ); ?>">
				<div class="swiper-wrapper">
					<?php foreach ( $settings['videos'] as $index => $item ) :
						...
						$link = ! empty( $item['link']['url'] ) ? $item['link'] : null;
						$link_key = 'slide_link_' . $index;
						if ( $link ) {
							$this->add_link_attributes( $link_key, $link );
							$this->add_render_attribute( $link_key, 'class', 'evhs-card-link' );
						}
						?>
						<div class="evhs-slide swiper-slide">
							<?php if ( $link ) : ?><a <?php echo $this->get_render_attribute_string( $link_key ); ?>><?php endif; ?>
								<div class="evhs-video-box">
									<?php if ( $video_url ) : ?>
										<video class="evhs-video" src="<?php echo esc_url( $video_url ); ?>" playsinline muted preload="metadata"></video>
									<?php else : ?>
										<video class="evhs-video" playsinline muted preload="metadata"></video>
									<?php endif; ?>
									...
									<div class="evhs-info-box">
										...
										<?php if ( ! empty( $item['button_text'] ) ) : ?><div class="evhs-info-subtitle evhs-button-label"><?php echo esc_html( $item['button_text'] ); ?></div><?php endif; ?>
									</div>
								</div>
